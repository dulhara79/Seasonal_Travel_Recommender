...
from server.schemas.orchestrator_schemas import OrchestratorAgent4OutpuSchema, OrchestratorAgent4InputSchema
...
prompt = ChatPromptTemplate.from_messages(
    [
        ("system", """You are a helpful assistant. You help peoples to get best recommendations to their travels in Sri Lanka. Extract the following information from the user's trip request:
        Do not include natural language, explanations, or any text outside of the JSON.
        - **location**: The destination city (e.g., Paris, Tokyo, Galle).
        ...
        Example: {{"location": "Paris", "start_date": "2025-09-06", "end_date": "2025-09-08", "no_of_traveler": 4, "budget": "medium", "user_preferences": ["culture", "food"], "type_of_trip": "leisure"}}
        Wrap the output in this format and provide no other text\n{format_instructions}"""),
        ("system", "Ensure the JSON is correctly formatted."),
        ("system", "Make sure to validate the dates and ensure they are in the correct format."),
        ("system", "If the user provides ambiguous information, ask clarifying questions."),
        ("system", "If the user does not provide certain details, use 'None' or an empty list as appropriate."),
        ("system", "Always respond with a JSON object, even if some fields are None or empty."),
...
MANDATORY_FIELDS = ["location", "start_date", "end_date", "no_of_traveler", "type_of_trip"]
FOLLOWUP_QUESTIONS = {
    "location": "🌍 Where would you like to travel?",
...
def run_llm_agent(state: OrchestratorAgent4InputSchema, chat_history: list = None, only_missing: list = None):
    ...
    if only_missing:
        sanitized_query = f"User already provided some info. Still need: {', '.join(only_missing)}"

    human_msg = HumanMessage(content=sanitized_query)
    chat_history.append(human_msg)

    result = agent_executor.invoke({"user_input": sanitized_query, "chat_history": chat_history})

    result_content = result["output"] if isinstance(result, dict) and "output" in result else str(result)
...
#             # If conversion fails, create a basic JSON structure
#             json_response = {
